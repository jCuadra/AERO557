function [ rKeep, vKeep, RSiteKeep, qHatKeep, qKeep, tau1Keep, tau3Keep ] = gaussOD(alpha, delta, siteInfo)
...
% Get RSite vectors
for i = 1:3:NUM_OBSV
    clear r2 eqn d1 d2 c rSite2
    RSite = getRSite(siteInfo, i);
    RSite = RSite(:, i:i+2);
    %RSite = [4054.881 3956.224 3905.073; 2748.195 2888.232 2956.935; 4074.237 4074.364 4074.430]
    % Get slant range unit vector
    qHat = getQHat(alpha(i:i+2), delta(i:i+2));
    %qHat = [.9473 .5742 .3007; .0155 .5747 .7399; .3201 .5830 .6018];
    % get r2
    tau = getTau(siteInfo, i+2, i);
    tau3 = getTau(siteInfo, i+2, i+1);
    tau1 = getTau(siteInfo, i, i+1);
